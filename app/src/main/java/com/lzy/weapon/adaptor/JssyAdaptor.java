...
public class JssyAdaptor extends BaseAdapter {
    private List<Jssy> list;
    private JssyActivity activity;
    private ImageView imageView;
    private TextView textView;

    public JssyAdaptor(List<Jssy> list,JssyActivity activity){
        this.list=list;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        textView.setText(list.get(position).getTitlesy());
...
//                .showImageOnFail(R.drawable.def).showImageOnLoading(R.drawable.def).build();
        ...
        if(list.get(position).getImg()!=null){
            ...
            ImageLoader.getInstance().displayImage(
                    list.get(position).getImg()
                            .substring(list.get(position).getImg().indexOf("src")+5
                                    ,list.get(position).getImg().indexOf("alt")-2),imageView, ImageOptionUtils.getWholeOptions());
        }else{
        ...
        jssy_ll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("title",list.get(position).getTitlesy());
                String imgstr="";
                if(list.get(position).getImg()!=null){
                    imgstr=list.get(position).getImg()
                            .substring(list.get(position).getImg().indexOf("src")+5
                                    ,list.get(position).getImg().indexOf("alt")-2);
                }
                intent.putExtra("img",imgstr);
                intent.putExtra("desc",list.get(position).getDescs());
